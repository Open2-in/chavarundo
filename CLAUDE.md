...
**Chavarundo** is a community road waste and garbage tracker for Kerala. It is a Next.js 15 (App Router) single-page app where the entire UI lives in one component — `components/LeafletWasteMap.tsx` loaded dynamically with `ssr: false` to avoid Leaflet SSR issues.

### Data Flow

1. **Real-time reads**: A single `onSnapshot` listener on the `waste_reports` Firestore collection drives the entire map. Reports are ordered by `createdAt` desc.
2. **Writing a report**: User clicks two points on the map → OSRM Router encodes the road segment as a polyline → Nominatim reverse-geocodes the origin → `/api/constituency` resolves AC/PC/LSGD via point-in-polygon against Open Data Kerala GeoJSON → `addDoc` to `waste_reports`.
...
- **All sub-components are internal functions** inside `LeafletWasteMap.tsx`: `RenderReports`, `ReportingOverlay`, `ReportDetailSheet`, `SubmitRouteForm`, `MapEventsHandler`, `RouteDisplay`, `SignInToVoteModal`, etc.
...
### Firestore Schema (`waste_reports` collection)
