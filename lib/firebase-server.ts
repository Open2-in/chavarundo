...
/**
 * Plain JSON-safe version of WasteReport for SSR props.
...
export async function getReport(id: string): Promise<WasteReport | null> {
  try {
    const token = await getAccessToken();
    const res = await fetch(`${restBase()}/waste_reports/${encodeURIComponent(id)}`, {
    ...
    return { id: docIdFromName(doc.name), ...data } as unknown as WasteReport;
  } catch (err) {
...
export async function getRecentReports(limit = 200): Promise<SerializedReport[]> {
  try {
    const rows = await runQuery({
      from: [{ collectionId: "waste_reports" }],
